defmodule PracLiveWeb.RatingLive.Index do
    use Phoenix.Component
    use Phoenix.HTML
    alias PracLiveWeb.RatingLive
    ...
    def list(assigns) do
        ...
        <%= for {game, index} <- Enum.with_index(@games) do %>
            <%= if rating = List.first(game.ratings) do %>
                <PracLiveWeb.RatingLive.Show.stars rating={rating} game={game} />
            <% else %>
                <.live_component module={RatingLive.FormComponent}
